...
class DDPG:
    ...
    def __init__(self, sess, state_dim, action_dim, action_max, action_min, is_batch_norm):
        self._tf_sess = sess

        self.num_states  = state_dim  #env.observation_space.shape[0]
        self.num_actions = action_dim #env.action_space.shape[0]
        
        self.critic_net = CriticNet(sess, self.num_states, self.num_actions) 
        self.actor_net  = ActorNet(sess,  self.num_states, self.num_actions)
        ...
        action_max = action_max #np.array(env.action_space.high).tolist()
        action_min = action_min #np.array(env.action_space.low).tolist()        
        action_bounds = [action_max, action_min] 
        self.grad_inv = grad_inverter(sess, action_bounds)
