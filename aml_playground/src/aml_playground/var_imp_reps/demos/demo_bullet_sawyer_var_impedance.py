...
from aml_playground.var_imp_reps.controller.sawyer_var_imp_reps import SawyerVarImpREPS
...
def reps():
    ...
    while i < exp_params['max_itr']:
        ...
        try:
            ...
            traj_data, reward = ps._eval_env.execute_policy(policy=policy,show_demo=False)

            mean_reward = ps._eval_env._penalty['total']

            force_penalty = ps._eval_env._penalty['force']

            print "Parameter found*****************************************: \t", np.mean(traj_data['params'], 0)
    ...
    'w_list':np.asarray(traj_data['params']),
    's_list':np.asarray(traj_data['contexts']),
