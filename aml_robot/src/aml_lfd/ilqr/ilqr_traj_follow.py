...
class DDP_TrajFollowClass():
    def __init__(self, idx, H, model, target_traj, reward, model_bias):
        ...
        self.magic_factors  = [.95, .8, .5, .2, .1, .05, .02]
    ...
    def simulate_f(self, index, x0, delta_u0, sim_time, model_bias=None, magic_factor=0, x1_star=0):
        ...
        Mq   = self.model[index]['inertia']
        Cq   = self.model[index]['gravity_comp'] 
        ...
        ## integration):
        ddgenq = np.dot(np.linalg.inv(Mq),(x0[self.idx['u_prev']]-Cq))
        ...
        #in a way this is wrong, since we are using the same jacobian for x + delta_x  and u + delta_u
       
        J = self.model[index]['jacobian']

        #added line
        ee_pose = np.dot(J, x0[self.idx['genq']][:7])

        #x1[self.idx['xyz']] = self.model[index]['ee_pos']

        x1[self.idx['xyz']] = ee_pose[:3]

        x1[self.idx['xyz_dot']] = np.dot(J, dgenq[:7])

        #x1[self.idx['q']] = self.model[index]['ee_ori']

        x1[self.idx['q']] = self.euler_to_q(ee_pose[3:])
    ...
    def linearized_dynamics(self, x0, u0, xt0, xt1, dt_sim, model_bias, magic_factor, x1star):
        ...
        for i in range(0,dx0.shape[0]-1): #[0],epsilon.shape[1]
            ...
            fx_t1m      = self.err_simulate(index=i-1, 
                                            dx0=dx0 - delta, 
                                            u0=u0, 
                                            xnom0=xt0, 
                                            xnom1=xt1, 
            ...
            fx_t1p      = self.err_simulate(index=i+1, 
                                            dx0=dx0 + delta, 
                                            u0=u0, 
                                            xnom0=xt0, 
                                            xnom1=xt1, 
...
def find_optimal_control_sequence(data, dt=0.01):
    ...
    data_pts = np.arange(1, num_data_pts, 2)
    data_partial = data[data_pts]
    #horizon length = length of the trajectory to be followed
    H = len(data_partial)#num_data_pts


    move_quaternion_target = data_partial[-1]['ee_ori']
    ...
    for i in range(H):
        target_move_state[idx['xyz']]  = data_partial[i]['ee_pos']
        target_move_state[idx['genq']] = np.hstack([data_partial[i]['position'], data_partial[i]['velocity']])
