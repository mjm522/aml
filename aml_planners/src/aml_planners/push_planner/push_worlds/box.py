...
class Box(object):
    ...
    def __init__(self, world, config):
        ...
        self._dyn_body = self._world.CreateDynamicBody(position=config['pos'], angle=config['ori'], fixedRotation=False)

        self._dyn_body.linearDamping = 0.#0.6
        self._dyn_body.angularDamping = 0.#0.05
        self._dyn_body.inertia = config['inertia']
        self._dyn_body.mass = config['mass']
        ...
        self._box = self._dyn_body.CreatePolygonFixture(box=self._config['box_dim'], density=1, friction=0.3, filter=b2Filter(groupIndex=-8,))
