...
class Model:
    ...
    def __init__(self, data):
        ...
        self._nb_states = 10 #Number of Gaussians in the GMM
        self._nb_var = 12 #Dimension of the datapoints in the dataset
        self.dt = 0 #time step 
...
class EM_GMM_GMR:
    ...
    def gmr(self, data_in, in_, out):
        ...
        kp_traj = np.zeros([nb_var_out, nb_data])
        kd_traj = np.zeros([nb_var_out, nb_data])
        ...
        for t in range(nb_data):
            ...
            for i in range(self.model._nb_states):
                ...
                weights[i,t] = self.model._priors[i] * self.gauss_PDF( data_in[:,t].reshape(9,1), self.model._mu[in_, i], tmp[in_,:])
            ...
            kp = np.zeros(nb_var_out)
            kd = np.zeros(nb_var_out)
            ...
            for i in range(self.model._nb_states):
                ...
                kp += weights[i,t]*sig_in_in[0,0]*(1./sig_out_in[:,0]) #(sig_out_in[:,0]*1./(sig_in_in[0,0]))
                kd += weights[i,t]*sig_in_in[1,1]*(1./sig_out_in[:,1]) #(sig_out_in[:,1]*1./(sig_in_in[1,1]))
        ...
        return data_out_mu, data_out_sigma, kp_traj, kd_traj

    def gauss_PDF(self, data, mu, sigma, check=False):
        """
